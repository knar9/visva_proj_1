...
function mapGroupToText(group){
  switch (group) {
    case 1:
      return "before 2005"
    case 2:
      return "between 2005 and 2010"
    case 3:
      return "between 2010 and 2015"
    case 4:
      return "between 2015 and 2020"
    case 5:
      return "after 2020"
    default:
      return "other"
...
function createArcDiagram(data) {
  ...
  // Add legends for groups
  const orderedGroups = Array.from(allGroups).sort((a, b) => a - b);
  var legendY = 20
  orderedGroups.forEach(group => {
    // Perform action on each element
    svg.append("circle").attr("cx",0).attr("cy",legendY).attr("r", 6).style("fill", color(group))
    svg.append("text").attr("x", 30).attr("y", legendY).text(mapGroupToText(group)).style("font-size", "15px").attr("alignment-baseline","middle")
    legendY += 20;
  });
  ...
  var labels = svg
    ...
    .attr("transform", function (d) {
      return "translate(" + x(d.title) + "," + (height - 15) + ")rotate(-45)";
    })
    .style("font-size", 6);
    ...
    .on("mouseover", function (d) {
      ...
      links
        .style("stroke", function (link_d) {
          return link_d.source === d.srcElement.__data__.id
            ? color(d.srcElement.__data__.grp)
            : "#b8b8b8";
        })
        .style("stroke-opacity", function (link_d) {
          return link_d.source === d.srcElement.__data__.id ? 1 : 0.2;
        })
        .style("stroke-width", function (link_d) {
          return link_d.source === d.srcElement.__data__.id ? 4 : 1;
        });
      labels
        .style("font-size", function (label_d) {
          return label_d.title === d.srcElement.__data__.title ? 16 : 2;
        })
        .attr("y", function (label_d) {
          return label_d.title === d.srcElement.__data__.title ? 10 : 0;
        });
    })
